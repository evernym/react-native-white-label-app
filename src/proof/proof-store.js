...
export function findMissingRequestedAttributes(
  ...
  Object.keys(proofRequest.requested_attributes).forEach((attrKey) => {
  ...
  if (proofRequest.requested_predicates) {
    Object.keys(proofRequest.requested_predicates).forEach((attrKey) => {
      const predicateClaimData = preparedProof.attrs[attrKey]
      if (
        (!predicateClaimData || !predicateClaimData[0]) &&
...
function findCredentialAttribute(credential: ClaimOfferPayload, label: string) {
  return credential.data.revealedAttributes.find(
  ...
  for (let credential of storedCredentials) {
    ...
    if (attribute.names && !usedCredentials.includes(credential.claimId)) {
      ...
      for (let label of attribute.names) {
        ...
        if (attributeName) {
          const credentialAttribute = findCredentialAttribute(
            credential,
            attributeName
          )
    ...
    if (attribute.name && !usedCredentials.includes(credential.claimId)) {
      let attributeName =
        credentialAttributes[caseInsensitive(attribute.name || '')]
      if (attributeName) {
        const credentialAttribute = findCredentialAttribute(
          credential,
          attributeName
...
export function convertIndyPreparedProofToAttributes(
  ...
  Object.keys(requestedAttributes).forEach((attributeKey) => {
    ...
    const labels: Array<string> = attribute.names
      ? attribute.names
      : attribute.name
      ? [attribute.name]
      : []
    ...
    if (revealedAttributes && revealedAttributes.length > 0) {
      ...
      const credentialsCanBeUsed = revealedAttributes.map(
        (revealedAttribute) => {
          ...
          return {
            ...
            data:
              ...
              caseInsensitiveMap &&
              revealedAttribute.cred_info.attrs[
                caseInsensitiveMap[caseInsensitive(label)]
              ],
            values: values,
            claimUuid:
              revealedAttribute && revealedAttribute.cred_info.referent,
      ...
      const credentialsExcludedByRestrictions = findCredentialsExcludedByAttributeRestrictions(
        storedCredentials,
        attribute,
        usedCredentials
      )
  ...
  if (requestedPredicates) {
    Object.keys(requestedPredicates).forEach((attributeKey) => {
      ...
      if (matchingCredentials && matchingCredentials.length > 0) {
        const credentialsCanBeUsed = matchingCredentials.map(
          (matchingCredential) => {
            // convert attrs props to lowercase
            ...
            return {
              ...
              data:
                ...
                caseInsensitiveMap &&
                matchingCredential.cred_info.attrs[
                  caseInsensitiveMap[caseInsensitive(predicate.name)]
                ],
              claimUuid:
                matchingCredential && matchingCredential.cred_info.referent,
        ...
        const credentialsExcludedByRestrictions = findCredentialsExcludedByPredicateRestrictions(
          storedCredentials,
          predicate,
          usedCredentials
        )
...
export function convertSelectedCredentialAttributesToIndyProof(
  ...
  Object.keys(proofRequest.requested_attributes).forEach((attributeKey) => {
    ...
    if (selectedAttribute) {
      ...
      const caseInsensitiveMap = buildCaseInsensitiveMap(
...
export function* generateProofSaga(action: GenerateProofAction): any {
  try {
    ...
    const matchingCredentials: IndyPreparedProof = yield* sortCredentials(
      matchingCredentialsJson
    )
    ...
    ] = findMissingRequestedAttributes(matchingCredentials, proofRequestData)
    ...
    const storedCredentials: Array<ClaimOfferPayload> = yield select(
      getReceivedCredentials
    )
...
export function* updateAttributeClaimAndSendProof(
  ...
  try {
    ...
    if (vcxResult && vcxResult.fail) {
      yield put(
        errorSendProofFail(action.uid, action.remoteDid, vcxResult.fail)
      )
      return
    }
    ...
    try {
      ...
      if (
        e.code === NO_POOL_OPEN ||
        e.code === CREDENTIAL_SCHEMA_NOT_FOUND ||
        e.code === CREDENTIAL_DEFINITION_NOT_FOUND
      ) {
        // if proof generation failed -> connect to pool ledger and retry (fetch fresh schema and cred def)
        yield call(
          connectToPoolAndRegenerateProof,
